 ...
 */
int
create_socket(const char *type, const char *port, int *sockfd, char *node, struct addrinfo *ainfo, int time)
{
    ...
    if (!strcmp(type, "TCP") || !strcmp(type, "tcp")) {
        ...
        if (!node) {
            create_tcp_server(sockfd, port, ainfo, time);
        } else {
 ...
 */
int
create_tcp_server(int *sockfd, const char *port, struct addrinfo *ainfo, int time)
{
    struct addrinfo hints, *res, *p;
    int rv;
    int yes=1;
