...
file_t *open_file(const char *filename, const char *flag)
{
    if (!filename) {
    ...
    if ( filebub->fp == NULL ) {
        fprintf(stderr, "Error opening this file: %s\n", strerror( errno ));
        return NULL;
    }
...
int read_file(file_t *filebub, void *buffer, size_t bytes)
{
    if (!buffer) {
...
int write_file(file_t *filebub, void *data, size_t len)
{
    if (!filebub->fp) {
        fprintf(stderr, "Invalid file pointer\n");
        return -1;
...
int delete_file(file_t *filebub)
{
...
// 29JUL2021
int crypter(file_t *filebub, char *key, size_t keylen)
{
    struct stat *attribs = get_attrib(filebub);
